...
function createLights() {
  hemisphereLight = new THREE.HemisphereLight(0xaaaaaa, 0x000000, 0.7);
  ambientLight = new THREE.AmbientLight(0xff8844, 0.3);
...
CoinsHolder.prototype.spawnCoins = function () {
  ...
  var d =
    ...
    game.planeDefaultHeight +
    (-1 + Math.random() * 2) * (game.planeAmpHeight - 20);
  var amplitude = Math.floor(Math.random() * 1);
  for (var i = 0; i < nCoins; i++) {
    var coin;
...
function loop() {
  ...
  if (game.status == "playing") {
    ...
    if (
      ...
      game.levelLastUpdate = Math.floor(game.distance); //были 2 буквы "с"
      game.level++;
      fieldLevel.innerHTML = Math.floor(game.level);
    ...
    coinsHolder.rotateCoins();
    airplane.mesh.position.y -= game.planeFallSpeed * deltaTime * 0.00001;

    ennemiesHolder.rotateEnnemies();
    sea.moveWaves();
    sea.mesh.rotation.z += 0.0005;
...
function updateEnergy() {
  ...
  energyBar.style.backgroundColor = game.energy < 50 ? "red" : "#68c3c0";
...
function updatePlane() {
  ...
  var targetY = normalize(mousePos.y, -0.75, 0.75, 25, 190);
...
function init(event) {
  ...
  document.addEventListener("mouseup", hangleMouseUp, false);
  document.addEventListener("touchend", hangleTouchEnd, false);
